(function () {
  ...
  var state = { bank: {}, sourceBank: {}, groupIndex: 0, answers: {}, result: null };
  var storageKey = 'futurepath_riasec_draft';
  var submitTimer = null;
  var accountSyncTimer = null;

  function $(id) { return document.getElementById(id); }
  function signedInId() {
    var user = window.FuturePathAuth && window.FuturePathAuth.getCurrentUser ? window.FuturePathAuth.getCurrentUser() : null;
  ...
  function saveDraft() {
    ...
    accountSyncTimer = window.setTimeout(function () {
      if (!window.FuturePathAuth || !window.FuturePathAuth.isSignedIn() || !window.FuturePathData) return;
      window.FuturePathData.setProfileValue('riasecDraft', {
        answers: Object.assign({}, state.answers),
        groupIndex: state.groupIndex,
        progress: progress
      });
      window.FuturePathData.setProfileValue('riasecProgress', progress);
      if (window.FuturePathData.saveProfile) {
        window.FuturePathData.saveProfile().catch(function () {});
      }
    }, 900);
  ...
  function loadDraft() {
    try {
      ...
      if (!draft.answers && window.FuturePathData && window.FuturePathData.getProfile) {
        draft = window.FuturePathData.getProfile().riasecDraft || {};
      }
  ...
  function parseQuestionBank(text) {
    ...
    while ((match = matcher.exec(text))) {
      ...
      bank[code].push({ id: code + number, text: match[3].trim(), aspect: Math.floor((number - 1) / 20) + 1 });
    }
  ...
  function selectQuestions(sourceBank) {
    ...
    GROUPS.forEach(function (group) {
      ...
      for (var aspect = 1; aspect <= 5; aspect += 1) {
        selected = selected.concat(randomSample(source.filter(function (question) { return question.aspect === aspect; }), 4));
      }
  ...
  function persistResult(result) {
    state.result = result;
    try { localStorage.setItem('futurepath_riasec_result:' + signedInId(), JSON.stringify(result)); } catch (error) {}
    if (window.FuturePathData && window.FuturePathData.setProfileValue) {
      window.FuturePathData.setProfileValue('riasecResult', result);
      window.FuturePathData.setProfileValue('riasecScores', result.scores);
      window.FuturePathData.setProfileValue('hollandCode', result.code);
      window.FuturePathData.setProfileValue('personality', buildPersonality(result));
      window.FuturePathData.setProfileValue('riasecProgress', {
        answered: result.answered,
        total: result.totalQuestions,
        completion: result.completion,
        completedAt: result.completedAt
      });
      if (window.FuturePathData.saveProfile && window.FuturePathAuth && window.FuturePathAuth.isSignedIn()) {
        window.FuturePathData.saveProfile().then(function () {
  ...
  function syncCareerMatches() {
    if (!window.FuturePathAuth || !window.FuturePathAuth.isSignedIn() || !window.FuturePathAuth.apiRequest) return Promise.resolve();
    return window.FuturePathAuth.apiRequest('/learning/assessments/run', { method: 'POST' }).then(function (matchResult) {
      var matches = matchResult && (matchResult.top5 || matchResult.results);
      if (!Array.isArray(matches)) return;
      state.result.careerMatches = matches.slice(0, 5);
      try { localStorage.setItem('futurepath_riasec_result:' + signedInId(), JSON.stringify(state.result)); } catch (error) {}
      if (window.FuturePathData && window.FuturePathData.setProfileValue) {
        window.FuturePathData.setProfileValue('careerMatches', matches.slice(0, 5));
        return window.FuturePathData.saveProfile ? window.FuturePathData.saveProfile() : undefined;
      }
      return undefined;
    }).then(function () {
  ...
  async function init() {
    ...
    $('retakeRiasec').addEventListener('click', function () {
      state.answers = {}; state.groupIndex = 0; state.result = null;
